...
class App extends Component {
  ...
  updateWinMessage(message, id) {
    ...
    this.setState({
      ...
      winner: (this.state.currentPlayer + 1) % 2,
  ...
  createEmptyPlayerStatus() {
    var newPlayerGameStatus = Array();
    this.state.playerGameStatus.forEach(function(element, index) {
      newPlayerGameStatus[index] = new Array();
  ...
  updateGameStatusTable(status) {
    var newPlayerGameStatus = this.createEmptyPlayerStatus();
    console.log("Table des joueurs : " + newPlayerGameStatus);
    switch (status) {
      case "victorious":
        newPlayerGameStatus[this.state.currentPlayer]["win"] =
          this.state.playerGameStatus[this.state.currentPlayer]["win"] + 1;
        newPlayerGameStatus[(this.state.currentPlayer + 1) % 2]["lost"] =
          this.state.playerGameStatus[(this.state.currentPlayer + 1) % 2][
            "lost"
          ] + 1;
        break;
      ...
      case "draw":
        newPlayerGameStatus[this.state.currentPlayer].draws =
          this.state.playerGameStatus[this.state.currentPlayer]["draws"] + 1;
    ...
    this.setState({
      player1GameStatus: newPlayerGameStatus[0],
      player2GameStatus: newPlayerGameStatus[1]
    });
  }
